...
// ══════════════════════════════════════════════════════════════════════
// 1.5) 基础属性：倒计时 + 每帧透明度变化（所有 mod 对象通用，实现在 src_mod.gml）
//      倒计时 > 0 时：倒计时 -1，image_alpha += mod_alpha_add
//      两个字段在 _OBJECT_CREATE 里用 VM_SetProp 设（默认 0 = 不生效）
//      典型用法：播一段动画 + 淡出（倒计时 30、mod_alpha_add = -1/30）
// ══════════════════════════════════════════════════════════════════════
mod_base_tick(id);
...
if (plant_id != "" && variable_global_exists("mod_card_vms") && ds_map_exists(global.mod_card_vms, plant_id)) {
	var _wvm = global.mod_card_vms[? plant_id];
