...
> 早期版本的 `LabMapCompiler.exe` 生成 `&&` / `||` 时跳转地址算错，表现是「条件像没生效、
> 整块被跳过」（更早的版本甚至会静默产出空块）。当前版本已修好，回归用例见 `_tests/ok_logic.txt`；
> 用之前确认自己手上是最新那份 exe（`bbb\FVM-reborn\mod\cards\` 与 `地图插件脚本工具demo\` 里的都要换）。
