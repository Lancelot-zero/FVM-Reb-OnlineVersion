...
- **触发条件看 VM 自己**（满足任意一条就进，没有别的门槛）：
  - **场上有它的实例**：卡片 / 武器 / 宝石 / 敌人 / 子弹 / 特效一样，该类单位一个实例都没有时挂载点不触发
    （`_OBJECT_CREATE` 都还没跑的新局、准备界面同理）。热重载换 bin、旧实例销毁都会自动跟上。
  - **卡片 mod 专属属性 `deck_active`**：这张卡在**战斗开始时的出战卡组**里 → 它的 VM **直接有效**，不用等种下
    （卡还在手槽 / 传送带上也照样收事件）。刷新点：进准备室、战斗开始、mod 加载 / 热重载。
  - **带 `__hookall__` 的 bin**：恒开 —— 调试用，见 `help.md` 文末「附：`__hookall__`」，**非必要不要用**。
...
- **`_OBJECT_*` 块里的读写一律本地执行**（两端各跑一遍，不发网络消息）—— Step 里高频写属性不会把网络打爆。
- **要跨端一致的字段，把字段名声明进 VM 级命名数组 `mod_sync`**（一般写在 `_OBJECT_CFG` 里）：
...
- 服务器**每 20 帧**遍历卡片 / 武器 / 宝石 / 敌人的实例，把 `mod_sync` 里**发生过变化**的字段打包成
  一条 `MSG_MODIFY_PROP` 推给客户端（子弹 / 特效不参与）。客户端收到后直接覆盖对应字段。
- **挂载点块（`_VM_*`）里的写仍按联网规则走**：本地实例（没有 net_id）直接改；有 net_id 的实例
  客户端跳过、服务器即时广播。
- 没进 `mod_sync`、两端又算岔了的字段会一直岔着 —— 要么保证它的输入两端一致，要么写进 `mod_sync`。
...
| 字段 / 全局 | 说明 |
|---|---|
| `net_player_id`（卡片实例字段） | 这张卡**是谁种的**：房主 `0`、客机从 `1` 起；单机 / 关卡自摆的卡是 `0`。`_OBJECT_CREATE` 里读到的是默认值 `0`，**`_OBJECT_STEP` 起才是正确值** |
| `global.mod_net_player_id`（全局） | **本机**在房间里的 id：房主 `0`、客机从 `1` 起；单机 / 未连接 / 已断开是 `0`。中继在**人数变化时会重发**，所以它总是最新的 |
| `global.net_player_number`（全局） | **房间里的人数**（房主 + 客机，至少 `1`）：单机 / 未连接 / 已断开是 `1`。中继在人数变化时和 `mod_net_player_id` 一起下发；读法 `VM_GetProp(0, "net_player_number")`（`0` = 读全局） |
