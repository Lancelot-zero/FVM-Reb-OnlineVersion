...
> 2. **触发条件看 VM 自己**，满足任意一条就叫醒它：
>    - **场上有它的实例** —— 卡片 / 武器 / 宝石 / 敌人 / 子弹 / 特效一视同仁：该类单位一个实例都没有时不触发
>      （`_OBJECT_CREATE` 还没跑过的新局同理）。热重载换 bin、旧实例销毁都会自动跟上。
>    - **卡片 mod 专属属性 `deck_active`**：这张卡在**战斗开始时的出战卡组**里 → 它的 VM「直接有效」，
>      不用等种下（卡还在手槽 / 传送带上也照样收事件）。刷新点：进准备室、战斗开始、mod 加载 / 热重载。
>    - **带 `__hookall__` 的 bin**：恒开（**特殊用法，非必要不要用**，见下方「附：`__hookall__`」）。
> 3. **卡片类事件不区分是谁的卡** —— 关卡自摆的卡、玩家角色、木板这些创建 / 销毁时也会广播；
>    想只处理自己这几张卡，在块里用 `VM_GetLastCreatedCard()` / `VM_GetLastDestroyedCard()` + `plant_id` 自己筛。
>
> **联机时属性怎么写（`mod_sync`）**
> 1. **`_OBJECT_*` 块里的读写一律本地执行、不发消息** —— 两端各跑一遍，跨端不即时同步。
>    这样 Step 里高频写属性不会把网络打爆。
> 2. **要跨端一致的字段，把名字声明进 VM 级命名数组 `mod_sync`**（写在 `_OBJECT_CFG` 里最省事）：
>    ```gml
>    VM_ArrayClear("mod_sync")
>    VM_ArrayAdd("mod_sync", "atk")          // 字段名 = 实例上的属性名
>    VM_ArrayAdd("mod_sync", "mod_phase")
>    ```
> 3. 服务器**每 20 帧**遍历一次卡片 / 武器 / 宝石 / 敌人的实例，把 `mod_sync` 里**发生变化**的字段
>    打包成一条消息推给客户端（子弹 / 特效不参与）。
> 4. **挂载点块（`_VM_*`）里的写仍然走联网规则**：本地实例直接改；有 net_id 的实例客户端跳过、服务器即时广播。
> 5. 没进 `mod_sync`、两端又算岔了的字段，就会一直岔着 —— 这类字段要么保证输入两端一致，要么写进 `mod_sync`。
