...
static std::vector<FuncDef> FUNC_DEFS = {
    ...
    {"VM_Ceil",                     1,  {PT_ANY},                           PT_ANY},      // 101 — 对数字向上取整，非数字返回 undefined
    {"VM_SpawnBatMouse",            2,  {PT_INT, PT_INT},                   PT_INT},      // 102 — 在指定格子生成蝙蝠鼠（含降落标记），返回实例 ID
    {"VM_GetTimeLimit",             0,  {},                                  PT_ANY},      // 103 — 获取关卡倒计时剩余帧数（无倒计时返回 undefined）(60帧1秒)
    {"VM_SetTimeLimit",             1,  {PT_INT},                            PT_ANY},      // 104 — 设置关卡倒计时帧数（无倒计时返回 undefined；联机自动同步）(60帧1秒)
    {"VM_SetDrawSlotEx",          8,    {PT_INT, PT_STRING, PT_INT, PT_INT, PT_FLOAT, PT_FLOAT, PT_FLOAT, PT_FLOAT},},      // 105 — 设置绘制槽位（含旋转缩放），角度/缩放传 -1 或 undefined 用默认值(0/1/1)
    {"VM_SetDrawSlotEx_front",    8,    {PT_INT, PT_STRING, PT_INT, PT_INT, PT_FLOAT, PT_FLOAT, PT_FLOAT, PT_FLOAT},},      // 106 — 设置前景绘制槽位（含旋转缩放），参数同 VM_SetDrawSlotEx
    {"VM_SetWaveAuto",            1,    {PT_INT},                             PT_INT,},      // 107 — 波次自动推进开关，1=自动（默认），0=关闭后完全不自动出怪，由脚本 VM_SetWave 控制
    {"VM_SetWave",                2,    {PT_INT, PT_INT},                     PT_INT,},      // 108 — 设置当前波次与子波次索引（0 开始），只改计数不出怪，变化触发对应波次钩子
    ...
    {"VM_GetHomingTarget",        1,    {PT_STRING},                          PT_INT,},      // 111 — 最左且血量最高的敌人 id（追踪索敌，param: 敌人类型，空串或all=任意；每帧全场只扫一次）
    ...
    {"VM_CanPlace",               3,    {PT_STRING, PT_INT, PT_INT},          PT_INT,},      // 121 — 按游戏正规种植规则判断某格能不能种这张卡（地形/障碍/水域莲叶/护盾层/底座卡/替换开关全都算），1=能 0=不能
    {"VM_CallFunc",               -1,},                                         // 122 — 按名字调用独立字典 global._VM_call_dict 里的函数（名字对编译器只是字符串，不校验）；第一个参数是函数名，后面是实参，最多 15 个
    {"VM_FuncExists",             1,    {PT_STRING},                          PT_INT,},      // 123 — 字典里有没有这个函数，1=有 0=没有（VM_CallFunc 的配套）
    {"VM_FuncDesc",               1,    {PT_STRING},                          PT_STRING,},      // 124 — 返回字典里登记的该函数说明字符串，没有登记就返回空串（VM_CallFunc 的配套）
    {"VM_SpriteExists",           1,    {PT_STRING},                          PT_INT,},      // 125 — 贴图现在真的可用吗（项目资源→VM临时缓存→VM永久缓存→全局缓存，并排除 get_load_sprite 的空白占位图），1=可用 0=不可用
    ...
    {"VM_ArrayExists",            1,    {PT_STRING},},                          // 127 — 全局二维表存不存在（存在且是数组）1=有 0=没有；表名指 global.<表名>
    {"VM_CellCount",              3,    {PT_STRING, PT_INT, PT_INT},},          // 128 — 表里第 i 列 j 行那一格的元素个数；表不存在 / 不是数组 / i,j 越界 → -1，所以 n = VM_CellCount(...) 之后直接 while 是安全的
    {"VM_CellItem",               4,    {PT_STRING, PT_INT, PT_INT, PT_INT},},      // 129 — 表里第 i 列 j 行那一格的第 k 个元素（k 从 0 起）；越界 → -1
    {"VM_CellContains",           4,    {PT_STRING, PT_INT, PT_INT, PT_ANY},},      // 130 — 表里第 i 列 j 行那一格有没有这个值，1=有 0=没有；越界 → -1
    {"VM_ArrayContains",          2,    {PT_STRING, PT_ANY},},                  // 131 — 命名一维数组里找值（和 VM_ArrayADD / VM_ArraySet / VM_ArrayGet 同一套，不是 enemy_array 那种全局二维表）：返回它第一次出现的下标（从 0 起）；没有这个值 / 数组不存在 → -1
    {"VM_InstArrayExists",        2,    {PT_INT, PT_STRING},},                  // 132 — 实例身上有没有这个一维数组，1=有 0=没有
    {"VM_InstArraySize",          2,    {PT_INT, PT_STRING},                  PT_INT,},      // 133 — 实例身上一维数组的长度；实例不存在 / 变量不存在 → -1
    ...
    {"VM_InstArrayAdd",           3,    {PT_INT, PT_STRING, PT_ANY},},          // 136 — 往实例身上一维数组的末尾追加一个值；失败 → -1
    {"VM_InstArrayDel",           3,    {PT_INT, PT_STRING, PT_INT},},          // 137 — 删掉实例身上一维数组的第 k 个元素；失败 → -1
    {"VM_InstArrayClear",         2,    {PT_INT, PT_STRING},},                  // 138 — 清空实例身上的一维数组；失败 → -1
    {"VM_InstArrayContains",      3,    {PT_INT, PT_STRING, PT_ANY},},          // 139 — 实例身上一维数组里有没有这个值，1=有 0=没有；实例/变量不存在 → -1
    {"VM_HomingBulletAdd",        11,   {PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY},},      // 140 — 追踪弹管理器：贴图,缩放,x,y,速度,伤害,可命中类型,销毁对象,mod名字,销毁贴图,模式（模式 0=只追全场最左的敌人，1=优先本行正前方 150 像素内血最多的敌人；每帧重新索敌，命中按 normal 走护盾，打中一个就消失）
    {"VM_DamageEnemy",            3,    {PT_INT, PT_ANY, PT_STRING},},          // 141 — 给敌人造成伤害（敌人id,伤害,伤害类型）：走敌人自己的受击事件，闪白 / 受击音效 / 护盾判定全都对，比直接改 hp 正确。伤害类型 normal=有盾只打盾，pierce=盾血一起掉，其它=无视护盾
    {"VM_DamageEnemyAsh",         3,    {PT_INT, PT_ANY, PT_STRING},},          // 142 — 灰烬伤害（敌人id,伤害,伤害类型）：伤害接得下就正常结算，接不下就一击必杀、原地换成 obj_mouse_ash_death（不看护盾，同原版大力神）
    {"VM_BulletScreenAdd_Ex",     18,   {PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY},},      // 143 — 屏幕弹幕管理器（带卡片效果）：贴图,总帧数,打击范围,缩放,动画速度,x,y,vx,vy,伤害,伤害计数,存活帧数,子弹类型,销毁对象,mod名字,伤害类型,标志数值,角度。标志数值=这颗子弹还能接受哪些类别的卡片效果：bit1=过火(换火弹贴图+放大1.8+音效)、bit2=解冻(换包子贴图+音效，不启用增益、冰冻帧清零、之后或上bit1)、4/8/16…=自定义（效果全看卡片字段）。子弹进格子中心带时与卡片的 bullet_flag 取且运算，>0 就应用并消位，同类卡只生效一次。角度=出生角度（后向子弹传180）
    {"VM_BulletScreenAdd_Exs",    20,   {PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY, PT_ANY},},      // 144 — 同 VM_BulletScreenAdd_Ex，最后多两个行渐变参数：目标行y（要拐去那一行的世界 y，不想渐变传 -1）、靠拢比例（每帧朝目标 y 靠拢的比例，原版水管弹用 0.15，别传 0）。渐变到位（|目标行y - y| <= 8）自动吸附并停掉渐变，之后是纯直线；途中经过的行照常结算。20 个参数都要写满
    ...
    {"VM_CatInRow",                1,   {PT_INT},},                                               // 146 — 该行第一只猫（obj_cat，海底图的螃蟹同对象）的实例 id；没有返回 -1
    ...
    {"VM_GetInstanceCount",        1,   {PT_STRING},},                                            // 148 — 某个对象类的实例个数（类名就是 obj_xxxx，和游戏里 object 名一致）；名字不存在返回 -1
    {"VM_GetInstanceAt",           2,   {PT_STRING, PT_INT},},                                    // 149 — 某个对象类第 k 个实例的 id（k 从 0 起）；越界 / 名字不存在返回 -1
};
